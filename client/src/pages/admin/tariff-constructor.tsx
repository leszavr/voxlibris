...
import { AlertCircle, Archive, Eye, Loader2, Plus, RotateCcw, Save, X } from "lucide-react";
...
export default function AdminTariffConstructorPage() {
  ...
  const addProductFeature = useMutation({
    mutationFn: () => apiRequest(`/api/commerce/admin/products/${selectedProductId}/features`, {
      method: "POST",
      body: JSON.stringify({ ...featureForm, ...featureValuePayload(featureForm), resetPeriod: featureForm.resetPeriod === "none" ? null : featureForm.resetPeriod, sortOrder: Number(featureForm.sortOrder || 0) }),
    }),
    onSuccess: async () => { await details.refetch(); toast({ title: "Фича добавлена" }); },
  });
...
function ProductsCard({ query, products, selectedProductId, pricingOnly, setPricingOnly, onSelect }: Readonly<{ query: ReturnType<typeof useQuery<Product[]>>; products?: Product[]; selectedProductId?: string; pricingOnly: boolean; setPricingOnly: (value: boolean) => void; onSelect: (product: Product) => void }>) {
  return <Card><CardHeader><CardTitle>Тарифы</CardTitle><CardDescription>Тарифные продукты и области действия.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="flex items-center gap-2 rounded-lg border p-3"><Switch checked={pricingOnly} onCheckedChange={setPricingOnly} /><Label>Только карточки /pricing</Label></div>{query.isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : null}{query.error ? <ErrorState error={query.error} /> : null}{products?.map((product) => <button key={product.id} type="button" onClick={() => onSelect(product)} className={`w-full rounded-lg border p-4 text-left hover:bg-muted/50 ${selectedProductId === product.id ? "border-primary" : ""}`}><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-medium">{product.title}</div><div className="flex gap-2"><Badge variant={product.status === "active" ? "default" : "secondary"}>{labelOf(statusLabels, product.status)}</Badge>{product.metadata?.isPopular ? <Badge variant="outline">Популярный</Badge> : null}</div></div><div className="text-sm text-muted-foreground">{product.code} · {labelOf(productTypeLabels, product.type)} · {labelOf(scopeTypeLabels, product.scopeType)}{product.scopeId ? `:${product.scopeId}` : ""}</div></button>)}{!query.isLoading && products?.length === 0 ? <EmptyState text={pricingOnly ? "Публичных платформенных тарифов пока нет." : "Тарифов пока нет."} /> : null}</CardContent></Card>;
}
...
function ProductDetailsCard(props: Readonly<{ details?: ProductDetails; isLoading: boolean; error: unknown; features: FeatureRegistryItem[]; priceForm: typeof priceDefaults; setPriceForm: (value: typeof priceDefaults) => void; addPrice: () => void; updatePrice: (price: Price) => void; featureForm: typeof featureDefaults; setFeatureForm: (value: typeof featureDefaults) => void; addFeature: () => void; updateFeature: (feature: ProductFeature) => void; deleteFeature: (id: string) => void }>) {
  ...
  return <Card><CardHeader><CardTitle>Цены, права и предпросмотр</CardTitle><CardDescription>Предпросмотр показывает, как платформенный тариф будет выглядеть на /pricing.</CardDescription></CardHeader><CardContent className="grid gap-6 lg:grid-cols-3"><div className="space-y-4"><h3 className="font-medium">Цены</h3>{details.prices.map((price) => <div key={price.id} className="flex items-center justify-between rounded-lg border p-3 text-sm"><span>{price.amountRub.toLocaleString("ru-RU")} ₽ / {labelOf(periodLabels, price.period)} · {price.status === "active" ? "активна" : "в архиве"}{price.isDefault ? " · по умолчанию" : ""}</span><Button size="sm" variant="outline" onClick={() => updatePrice(price)}>{price.status === "active" ? "В архив" : "Активировать"}</Button></div>)}<div className="grid grid-cols-3 gap-2"><Input type="number" value={priceForm.amountRub} onChange={(e) => setPriceForm({ ...priceForm, amountRub: e.target.value })} /><Select value={priceForm.period} onValueChange={(period) => setPriceForm({ ...priceForm, period: period as Period })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["one_time", "week", "month", "quarter", "year"].map((item) => <SelectItem key={item} value={item}>{labelOf(periodLabels, item)}</SelectItem>)}</SelectContent></Select><Button onClick={addPrice} className="gap-2"><Plus className="h-4 w-4" />Цена</Button></div></div><div className="space-y-4"><h3 className="font-medium">Права тарифа</h3>{details.features.map((feature) => <ProductFeatureRow key={feature.id} feature={feature} features={features} isEditing={editingFeatureId === feature.id} onEdit={() => setEditingFeatureId(feature.id)} onCancel={() => setEditingFeatureId(null)} onSave={(next) => { updateFeature(next); setEditingFeatureId(null); }} onArchive={() => deleteFeature(feature.id)} />)}<FeatureValueEditor form={featureForm} setForm={setFeatureForm} features={features} /><Button onClick={addFeature} disabled={!featureForm.featureKey} className="gap-2"><Plus className="h-4 w-4" />Право</Button></div><PricingPreview product={details} /></CardContent></Card>;
}
...
function ProductFeatureRow(props: Readonly<{ feature: ProductFeature; features: FeatureRegistryItem[]; isEditing: boolean; onEdit: () => void; onCancel: () => void; onSave: (feature: ProductFeature) => void; onArchive: () => void }>) {
  const { feature, features, isEditing, onEdit, onCancel, onSave, onArchive } = props;
  ...
  return <div className={`flex items-center justify-between rounded-lg border p-3 text-sm ${feature.isActive ? "" : "bg-muted/40"}`}><div><div className="flex items-center gap-2"><span className="font-medium">{feature.label}</span>{feature.isActive ? null : <Badge variant="secondary">Архив</Badge>}{feature.isHighlighted ? <Badge variant="outline">Highlight</Badge> : null}<SupportBadge featureKey={feature.featureKey} /></div><div className="text-muted-foreground">{feature.featureKey} · {labelOf(valueTypeLabels, feature.valueType)} · {formatFeatureValue(feature)}</div>{support.status !== "implemented" ? <div className="mt-1 text-xs text-amber-700">{support.note}</div> : null}</div><div className="flex gap-2"><Button size="icon" variant="outline" onClick={() => { setForm(formFromFeature(feature)); onEdit(); }}><Eye className="h-4 w-4" /></Button>{feature.isActive ? <Button size="icon" variant="outline" onClick={onArchive}><Archive className="h-4 w-4" /></Button> : <Button size="icon" variant="outline" onClick={() => onSave({ ...feature, isActive: true })}><RotateCcw className="h-4 w-4" /></Button>}</div></div>;
}
...
function FeatureValueEditor({ form, setForm, features }: Readonly<{ form: typeof featureDefaults; setForm: (value: typeof featureDefaults) => void; features: FeatureRegistryItem[] }>) {
  ...
  return <div className="grid gap-3 md:grid-cols-2"><div className="space-y-2"><div className="relative"><Select value={form.featureKey || "none"} onValueChange={selectFeature}><SelectTrigger className="h-auto min-h-10 pr-9 text-left"><SelectValue placeholder="Ключ права" />{selectedFeature ? <span className="block text-xs text-muted-foreground">{selectedFeature.title}</span> : null}</SelectTrigger><SelectContent className="max-h-[min(420px,calc(100vh-160px))] overflow-y-auto"><SelectItem value="none" disabled>Ключ права</SelectItem>{features.map((feature) => <SelectItem key={feature.key} value={feature.key}><span className="flex flex-col gap-0.5"><span>{feature.title}</span><span className="text-xs text-muted-foreground">{feature.key}</span><span className="text-xs text-muted-foreground">{supportStatusLabels[getCommerceFeatureSupport(feature.key).status]}</span></span></SelectItem>)}</SelectContent></Select>{form.featureKey ? <Button type="button" size="icon" variant="ghost" aria-label="Очистить ключ права" className="absolute right-1 top-1 h-8 w-8" onClick={() => setForm({ ...form, featureKey: "" })}><X className="h-4 w-4" /></Button> : null}</div>{support ? <div className="flex flex-wrap items-center gap-2 text-xs"><SupportBadge featureKey={form.featureKey} />{support.status !== "implemented" ? <span className="text-amber-700">{support.note}</span> : <span className="text-muted-foreground">{support.note}</span>}</div> : null}</div><Input placeholder="Название" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} /><Select value={form.valueType} onValueChange={(valueType) => setForm({ ...form, valueType: valueType as ValueType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="boolean">{valueTypeLabels.boolean}</SelectItem><SelectItem value="integer">{valueTypeLabels.integer}</SelectItem><SelectItem value="string">{valueTypeLabels.string}</SelectItem><SelectItem value="json">{valueTypeLabels.json}</SelectItem></SelectContent></Select>{form.valueType === "boolean" ? <div className="flex items-center gap-2"><Switch checked={form.valueBool} onCheckedChange={(valueBool) => setForm({ ...form, valueBool })} /><Label>Значение</Label></div> : null}{form.valueType === "integer" ? <Input placeholder="Числовое значение" type="number" value={form.valueInt} onChange={(e) => setForm({ ...form, valueInt: e.target.value })} /> : null}{form.valueType === "string" ? <Input placeholder="Текстовое значение" value={form.valueText} onChange={(e) => setForm({ ...form, valueText: e.target.value })} /> : null}{form.valueType === "json" ? <Textarea placeholder='JSON, например {"limit":10}' value={form.valueJson} onChange={(e) => setForm({ ...form, valueJson: e.target.value })} /> : null}<Select value={form.resetPeriod} onValueChange={(resetPeriod) => setForm({ ...form, resetPeriod })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">{resetPeriodLabels.none}</SelectItem><SelectItem value="day">{resetPeriodLabels.day}</SelectItem><SelectItem value="week">{resetPeriodLabels.week}</SelectItem><SelectItem value="month">{resetPeriodLabels.month}</SelectItem><SelectItem value="year">{resetPeriodLabels.year}</SelectItem></SelectContent></Select><Input placeholder="Порядок сортировки" type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: e.target.value })} /></div>;
}

function SupportBadge({ featureKey }: Readonly<{ featureKey: string }>) {
  const support = getCommerceFeatureSupport(featureKey);
  return <Badge variant={support.status === "implemented" ? "default" : "secondary"}>{supportStatusLabels[support.status]}</Badge>;
}
