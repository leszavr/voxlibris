...
import { AlertCircle, Archive, Eye, Loader2, Plus, RotateCcw, Save } from "lucide-react";
...
function ProductFeatureRow(props: Readonly<{ feature: ProductFeature; features: FeatureRegistryItem[]; isEditing: boolean; onEdit: () => void; onCancel: () => void; onSave: (feature: ProductFeature) => void; onArchive: () => void }>) {
  ...
  return <div className={`flex items-center justify-between rounded-lg border p-3 text-sm ${feature.isActive ? "" : "bg-muted/40"}`}><div><div className="flex items-center gap-2"><span className="font-medium">{feature.label}</span>{feature.isActive ? null : <Badge variant="secondary">Архив</Badge>}{feature.isHighlighted ? <Badge variant="outline">Highlight</Badge> : null}</div><div className="text-muted-foreground">{feature.featureKey} · {labelOf(valueTypeLabels, feature.valueType)} · {formatFeatureValue(feature)}</div></div><div className="flex gap-2"><Button size="icon" variant="outline" onClick={() => { setForm(formFromFeature(feature)); onEdit(); }}><Eye className="h-4 w-4" /></Button>{feature.isActive ? <Button size="icon" variant="outline" onClick={onArchive}><Archive className="h-4 w-4" /></Button> : <Button size="icon" variant="outline" onClick={() => onSave({ ...feature, isActive: true })}><RotateCcw className="h-4 w-4" /></Button>}</div></div>;
}
