...
function registryFormFromFeature(feature: FeatureRegistryItem): typeof registryDefaults {
  return {
    key: feature.key,
    title: feature.title,
    ...
    defaultBool: feature.defaultBool ?? false,
    defaultInt: feature.defaultInt == null ? "" : String(feature.defaultInt),
    defaultText: feature.defaultText ?? "",
    defaultJson: feature.defaultJson == null ? "" : JSON.stringify(feature.defaultJson, null, 2),
...
export default function AdminTariffConstructorPage() {
  ...
  return <AdminLayout><div className="container mx-auto space-y-6 px-4 py-8">
    <div><h1 className="text-3xl font-bold">Конструктор тарифов</h1><p className="text-muted-foreground">Продукты, цены и права доступа.</p></div>
    <Tabs defaultValue="products"><TabsList><TabsTrigger value="products">Тарифы</TabsTrigger><TabsTrigger value="features">Реестр прав</TabsTrigger></TabsList>
      ...
      <TabsContent value="features" className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]"><FeatureRegistryList query={features} selectedKey={registryForm.key} onSelect={(feature) => setRegistryForm(registryFormFromFeature(feature))} /><FeatureRegistryEditor form={registryForm} setForm={setRegistryForm} save={() => saveRegistry.mutate()} saving={saveRegistry.isPending} reset={() => setRegistryForm(registryDefaults)} /></TabsContent>
    </Tabs>
  </div></AdminLayout>;
}
