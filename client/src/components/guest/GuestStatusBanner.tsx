...
function navigateToGuestSession(
  setLocation: (path: string) => void,
  session: { book: { bookId: string } | null }
) {
  if (session.book) {
...
interface GuestActiveStateProps {
  accessCode: string;
  hasBook: boolean;
  bookId?: string;
...
function GuestActiveState({
  ...
  onOpenLibrary,
  onToggleRestore,
}: Readonly<GuestActiveStateProps>) {
  return (
    <>
...
function GuestInitState({
  isLoading,
  isSubmitting,
  onStart,
  onToggleRestore,
}: Readonly<GuestInitStateProps>) {
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
...
export function GuestStatusBanner() {
  ...
  async function handleGuestStart() {
    ...
    try {
      ...
      navigateToGuestSession(setLocation, session);
    } catch (e) {
    ...
    <div className="bg-muted/50 rounded-xl p-3 space-y-3">
      {guest ? (
        <GuestActiveState
          ...
          onCopyCode={() => void handleCodeCopy()}
          onOpenReader={() => setLocation(`/guest/reader/${book?.bookId ?? ""}`)}
          onOpenLibrary={() => setLocation("/guest/library")}
          onToggleRestore={() => setShowRestore((prev) => !prev)}
        />
      ) : (
        <GuestInitState
          isLoading={isLoading}
          isSubmitting={isSubmitting}
          onStart={() => void handleGuestStart()}
          onToggleRestore={() => setShowRestore((prev) => !prev)}
        />
      )}
