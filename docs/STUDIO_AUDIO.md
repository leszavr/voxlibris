...
Хранение намерения закрытия стрима при перезагрузке страницы или реконнекте WebSocket.

### Реализация

**Файл:** `server/lib/studio-stream-intent-store.ts`

Двухуровневое хранение:
- **Локальный кэш** (`Map`) — быстрый доступ
- **Redis** — распределённое хранение между нодами

### Типы намерений

| Намерение | Описание |
|-----------|----------|
| `pause` | Приостановить трансляцию |
...
```bash
# TTL записи (секунды), по умолчанию 1800 (30 минут)
STUDIO_STREAM_INTENT_TTL_SECONDS=1800
```

### Fallback

Если Redis недоступен — работает на локальном кэше.
